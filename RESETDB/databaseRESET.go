...
DROP TABLE IF EXISTS friend_request;
	...
	createTables := `
CREATE TABLE user(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, uuid VARCHAR(255),  profilePicture VARCHAR(255), email VARCHAR(64), username VARCHAR(20), password VARCHAR(255));
...
CREATE TABLE post(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title VARCHAR(32), content VARCHAR(255), user_id INTEGER, community_id INTEGER, created TIMESTAMP, FOREIGN KEY (user_id) REFERENCES user(id), FOREIGN KEY (community_id) REFERENCES community(id));
...
CREATE TABLE community(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, name VARCHAR(32), following INTEGER NOT NULL);
